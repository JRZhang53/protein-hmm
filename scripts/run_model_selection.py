...
def main() -> None:
    ...
    val_residues = sum(len(seq) for seq in val_sequences) or 1
    test_residues = sum(len(seq) for seq in test_sequences) or 1

    base_kwargs = {key: value for key, value in config.models["unsupervised"].items() if key != "num_states"}
    pseudocounts = config.models.get("model_selection", {}).get("pseudocount_sweep")

    results = []
    for num_states in config.models["model_selection"]["candidate_states"]:
        sweep_values = pseudocounts if pseudocounts else [base_kwargs.get("pseudocount", 1e-3)]
        for pseudocount in sweep_values:
            kwargs = dict(base_kwargs)
            kwargs["pseudocount"] = pseudocount
            model = DiscreteHMM(num_states=num_states, **kwargs)
            ...
            row = {
                ...
                "train_log_likelihood_per_residue": _per_residue(train_log_likelihood, train_residues),
                "val_log_likelihood_per_residue": _per_residue(val_log_likelihood, val_residues),
                "test_log_likelihood_per_residue": _per_residue(test_log_likelihood, test_residues),
                "bic": bic_score(train_log_likelihood, model.parameter_count(), train_residues),
                "training_diagnostics": training_diagnostics(model.training_history, train_residues),
                "training_log_likelihoods": list(model.training_history.log_likelihoods),
